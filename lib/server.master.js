	...
	sys.forkReload = function(){
		...
		master.each(siteIdToWorkerInfo, function(siteId, widsInfo) {
			for (; ((widsInfo.cpuLen - widsInfo.wids.length||0)>=1);) {
				...
				//
				let pkg = path.join(options.workerFile, 'package.json');

				let npmRunCommand = '';

				if (fs.existsSync(pkg)) {
					let pkgOptions = require(pkg);
					let scripts = pkgOptions.scripts || {};
					if (scripts.ddvstart) {
						npmRunCommand = scripts.ddvstart || npmRunCommand ;
					}else if (scripts.start) {
						npmRunCommand = scripts.start || npmRunCommand ;
					}else if (scripts.run) {
						npmRunCommand = scripts.run || npmRunCommand ;
					}
				}
				if (!npmRunCommand) {
					console.error('没有找到命令');
					return ;
				}
				...
				//建立Which
				let npmWhich = NpmWhich(options.workerFile);
				//解析命令行为数组
				let npmRunArgs = npmRunCommand.split(' ');
					//提取命令
					npmRunCommand = npmRunArgs.shift();
				let commandFile = 'index.js';
					//获取命令的文件名
				let npmRunCommandBasename = path.basename(npmRunCommand);
					//判断命令是否为node
					if (npmRunCommandBasename==='node'||npmRunCommandBasename==='node.exe') {
						//因为不能node 运行node 所以截取掉
						commandFile = npmRunArgs.shift();
					}else{
						//获取命令行文件地址，建议使用同步模式，
						//由npmWhich原理可以看出，同步模式减少意外
						commandFile = npmWhich.sync(npmRunCommand, npmRunoptions) || commandFile;
					}
					//补充当前的参数
					npmRunArgs.push.apply(npmRunArgs, args);


				//启动子进程
				let worker = fork(commandFile, npmRunArgs, npmRunoptions);

			...
			new Promise(function(resolve, reject) {
				...
				try{
					...
					switch(true){
						...
						case stat.isFIFO():
						case stat.isSocket():
							fs.unlink(master._serverProxyListenPort, function unlinkCb(e){
				...
				new Promise(function(resolve, reject) {
					//修改管道权限
					fs.access(master._serverProxyListenPort,  fs.constants.F_OK | fs.constants.R_OK | fs.constants.W_OK, function(e) {
						return e?reject(e):resolve();
					});
				}).then(function(){
						new Promise(function(resolve, reject) {
							//赋予管道权限
							fs.chmod(master._serverProxyListenPort, 0o666, function chmodCb(e){
								return e?reject(e):resolve();
							});
						}).then(function(){
							resolve();

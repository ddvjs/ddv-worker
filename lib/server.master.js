...
module.exports = function masterServerInit (master) {
  ...
  sys.loadSiteListsAndReload = function (options) {
    ...
    master.each(siteIdToServerConfig, function (siteId, options) {
      ...
      master.each(options.listen, function (listenId, listen) {
        // 默认type为tcp协议
    ...
    master.each(siteIdToServerConfig, (siteId, options) => {
    ...
    // 删除原来不存在的
    master.each(listenToSiteId, (key) => {
    ...
    // 插入新的
    master.each(listenToSiteIdT, (key) => {
      listenToSiteId[key] = listenToSiteIdT[key]
    })

    // 删除原来不存在的
    master.each(listenLists, (key) => {
      if (!listenListsT[key]) {
    ...
    // 插入新的
    master.each(listenListsT, (key) => {
      listenLists[key] = listenListsT[key]
    })

    // 删除原来不存在的
    master.each(siteIdToWorkerInfo, (key) => {
    ...
    // 插入新的
    master.each(siteIdToWorkerInfoT, (key) => {
  ...
  sys.forkReload = function () {
    ...
    master.each(siteIdToWorkerInfo, function (siteId, widsInfo) {
      while (((widsInfo.cpuLen - widsInfo.wids.length) || 0) >= 1) {
        ...
        master.extend(true, options.workerOptions, {
          silent: true,
          env: {
            DDV_WORKER_PROCESS_TYPE: 'worker',
            DDV_WORKER_PROCESS_WORKER_ID: workerId
  ...
  sys.sendSocketToWorker = function sendSocketToWorker (headers, typePortIp, listenInfo, socket, bufferByBodyEnd) {
    ...
    if ((widsInfo = (siteIdToWorkerInfo[siteId])) && widsInfo.wids && widsInfo.wids.length > 0) {
      ...
      if ((worker = (master && workers[workerId])) && (worker && master.type(worker, 'object') && master.type(worker.send, 'function'))) {

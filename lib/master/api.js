...
master.onWorkerCall('getServerConf', function getServerConfCb (data, handle, worker) {
  ...
  var siteId = worker && worker.id && workerIdToSiteId[worker.id]
  return siteId && siteIdToServerConfig[siteId] ? Promise.resolve({serverConf: siteIdToServerConfig[siteId]}) : Promise.reject(new master.MasterError('worker not find', null, 'WORKER_NOT_FIND'))
})

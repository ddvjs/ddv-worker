...
master.on('daemon::event::callMaster', (res, handle) => {
  ...
  Promise.resolve().then(() => {
    ...
    if (onDaemonCalls[res.name] instanceof Promise) {
      PromiseRes.push(onDaemonCalls[res.name](res.message, handle, callback))
    } else {
      onDaemonCalls[res.name](res.message, handle, callback)
    }
    callback = void 0
    return Promise.race(PromiseRes)
  }).then(({id, message, handle, options}) => {
    master.sendToDaemon('callMasterRes', { id: res.id, message }, handle, options)
  }, e => {
...
master.on('worker::event::callMaster', (res, handle, worker) => {
  ...
  Promise.resolve().then(() => {
    ...
    if (onWorkerCalls[res.name] instanceof Promise) {
      PromiseRes.push(onWorkerCalls[res.name](res.message, handle, callback))
    } else {
      onWorkerCalls[res.name](res.message, handle, callback)
    }
    callback = void 0
    return Promise.race(PromiseRes)
  }).then(({id, message, handle, options}) => {
    master.sendToWorker(worker, 'callMasterRes', { id: res.id, message }, handle, options)
  }, e => {

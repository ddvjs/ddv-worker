...
master.sendToWorker = function masterSendToWorker (worker, type, message, handle, options, callback) {
  ...
  return new Promise((resolve, reject) => {
    ...
    body.type = type
    if (worker.send(body, handle, options)) {
      resolve()
    } else {
      let e = new master.MasterError('master send worker fail')
      e.type = 'sendToWorker'
    ...
    body = worker = type = message = handle = options = void 0
    return r
  }).then(res => {
...
master.sendToDaemon = function masterSendToDaemon (type, message, handle, options, callback) {
  ...
  return new Promise((resolve, reject) => {
    ...
    body.id = util.createNewPid()
    body.type = type
    if (process.send(body, handle, options)) {
      resolve()
    } else {
      let e = new master.MasterError('master send worker fail')
      e.type = 'sendToDaemon'
...
master.callDaemon = function (name, data, handle, options, callback) {
  ...
  return new Promise((resolve, reject) => {
    var body, callId
    body = Object.create(null)
    body.id = callId = util.createNewPid()
    body.name = name
    body.data = data
    processPromise[body.id] = [resolve, reject, this]
    // 发送
    master.sendToDaemon('callDaemon', body, handle, options).catch(e => {
      if (processPromise[callId] && util.isFunction(processPromise[callId][1])) {
        processPromise[callId][1].call(processPromise[callId][2] || this, e)
        delete processPromise[callId]
      }
    })
    name = data = handle = options = body = void 0
  }).then((res) => {
    util.isFunction(callback) && callback(null, res)
    callback = void 0
    return res
...
master.callWorker = function (worker, name, data, handle, options, callback) {
  ...
  return new Promise((resolve, reject) => {
    ...
    body.data = data
    processPromise[body.id] = [resolve, reject, this]
    master.sendToWorker(worker, 'callWorker', body, handle).catch(e => {
      if (processPromise[callId] && util.isFunction(processPromise[callId][1])) {
        processPromise[callId][1].call(processPromise[callId][2] || this, e)
        delete processPromise[callId]
      }
    })
    worker = name = data = handle = options = body
  }).then((res) => {
    util.isFunction(callback) && callback(null, res)
    callback = void 0
    return res

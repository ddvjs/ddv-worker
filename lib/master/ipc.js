...
master.on('daemon::event::callMaster', (res, handle) => {
  ...
  Promise.resolve().then(() => {
    ...
    if (onDaemonCalls[res.name] instanceof Promise) {
      PromiseRes.push(onDaemonCalls[res.name](res.data, handle, callback))
    } else {
      onDaemonCalls[res.name](res.data, handle, callback)
    }
    callback = void 0
...
master.on('worker::event::callMaster', (res, handle, worker) => {
  ...
  Promise.resolve().then(() => {
    ...
    if (onWorkerCalls[res.name] instanceof Promise) {
      PromiseRes.push(onWorkerCalls[res.name](res.data, handle, callback))
    } else {
      onWorkerCalls[res.name](res.data, handle, callback)
    }
    callback = void 0
    return Promise.race(PromiseRes)
  }).then(({id, data, handle, options}) => {

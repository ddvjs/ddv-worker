  ...
  return Promise((resolve, reject) => {
    ...
    if (worker.isForkInitEnd) {
      ...
      var timeout = setTimeout(() => {
        ...
        reject(new master.MasterError('master wait worker timeout', null, 'MASTER_WAIT_WORKER_TIMEOUT', 'sendToWorker'))
      }, 10 * 1000)
      var evnetFn = (res, handle, fromWorker) => {
        if ((fromWorker.id || fromWorker.workerId) === (worker.id || worker.workerId)) {
...
master.on('worker::event::workerSendToWorker', function workerSendToWorker (body, handle, worker) {
  master.sendToWorker(body.toWorkerId, body.type, body.message, (state, message) => {
    ...
    master.sendToWorker(worker.id, 'workerSendToWorkerCallback', {id: body.id, message, isCallback: true}, handle)
  }, ({name, type, stack, handle}) => {
    master.sendToWorker(worker.id, 'workerSendToWorkerCallback', {id: body.id, errorName: name, errorType: type, errorStack: stack, isCallback: true}, handle)
  })
})
...
process.nextTick(() => {
  master.sendToDaemon('isForkInitEnd')
})

...
module.exports = function masterInit (master) {
  ...
  master.sendToWorker = function masterSendToWorker (worker, type, message, handle, options, callback) {
    if (callback === void 0 && master.isFunction(options)) {
      callback = options
      options = void 0
    }
    if (callback === void 0 && master.isFunction(handle)) {
      callback = handle
      options = handle = void 0
    }
    if (callback === void 0 && master.isFunction(message)) {
      callback = message
  ...
  master.callWorker = function (worker, name, data, handle, options, callback) {
    ...
    if (callback === void 0 && master.isFunction(data)) {
      callback = data
      options = data = void 0
    }
    var body
  ...
  sys.initByDaemon = function () {
    ...
    // 发信息给守护进程，启动被初始化
    master.callDaemon('init', {}, (state, message, handle) => {
      if (state === true) {
  ...
  sys.daemonPing = function daemonPing () {
    ...
    try {
      // 和主进程ping
      master.callDaemon('ping', {}, (state, message, handle) => {
        // 最后的时间
        if (state) {
          sys.daemonPingLastTime = master.now()
        } else {

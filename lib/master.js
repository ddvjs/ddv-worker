...
module.exports = function masterInit(master){
	...
	//系统内部配置
	const sys = master.__sys_private__ = b.inherit(null) ;
	//服务器workerid转serverid
	const workerIdToSiteId = sys.workerIdToSiteId = b.inherit(null) ;
	//服务器id转workerInfo
	const siteIdToSiteConfig = sys.siteIdToSiteConfig = b.inherit(null) ;
	//服务器workerid转pid
	const workerIdToPId = sys.workerIdToPId = b.inherit(null) ;
	/**
	 * [sendToWorker 发送子进程]
	 * @author: 桦 <yuchonghua@163.com>
	 * @DateTime 2016-11-14T14:16:23+0800
	 * @param    {string|object}          worker   [worker|worker_id]
	...
	master.sendToWorker = function masterSendToWorker(worker, type, message, handle, options, callback){
		...
		if (worker&&(b.type(worker, 'number')||b.type(worker, 'string'))) {
			worker = (master&&master.workers[worker]) || worker ;
		}
		if (!(worker&&b.type(worker, 'object')&&b.type(worker.send, 'function'))) {
	...
	master.on('worker::event::workerSendToWorker', function workerSendToWorker(body, handle, worker){
		master.sendToWorker(body.to_worker_id, body.type, body.message,function(state,message){
	...
	master.on('worker::call::init',function getSiteInfoCb(data, handle, callback, worker){
		...
		if (worker&&worker.id&&workerIdToSiteId[worker.id]) {
			...
			res.server_id = workerIdToSiteId[res.worker_id] ;
			res.serverConf = siteIdToSiteConfig[res.server_id] ;
			res.serverGuid = master.serverGuid ;
			res.site_id = res.serverConf.site_id;
			res.is_virtual_host = sys.is_virtual_host;
			callback(true, res);
		}else{
			callback(false, 'site_id not find');
		}
	});
	//ping 子进程ping
	...
	master.on('daemon::call::kill',function kill(data, handle, callback){
		var pids, q = b.queue();
			...
			//循环拼接队列
			b.each(workerIdToSiteId, function(worker_id) {
				q.push(true, function sendKillToWorker(next){
					master.callWorker(worker_id, 'kill', function callback(state, data){
						var process_id = state?data.pid:(workerIdToPId[worker_id]);
						if (process_id) {
							pids[pids.length] = {
						...
						next();
						data = worker_id = process_id = undefined;
					});
				});
			});
			//子进程杀掉完毕
	...
	sys.daemonPing = function daemonPing() {
		if ((b.now() - (sys.daemonPingLastTime||0)) > (sys.daemonPingTimeOut*2) ) {
	...
	sys.workerEventBind = function(worker){
		b.each(['message','exit','close','error'], function(index, type) {
	...
	//日志对象
	sys._log = b.inherit(null);
	//文件指针存储
	sys._log.pathToFd = b.inherit(null);
	//队列
	sys._log.bufferQueue = b.inherit(null);
	//队列锁
	sys._log.bufferLock = b.inherit(null);
	//删除文件指针
	...
	sys._log.onStdoutData = function(data){
		...
		sys._log.write(this.log_path_output, data, 0, data.length);
		sys._log.write(this.log_path_all, data, 0, data.length);
	};
	sys._log.onStderrData = function(data){
		process.stderr.write(data);

		sys._log.write(this.log_path_error, data, 0, data.length);
		sys._log.write(this.log_path_all, data, 0, data.length);
	};
	/**
	 * [run 运行管理进程的服务]

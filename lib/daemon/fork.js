...
Object.assign(require('ddv-worker').prototype, {
  ...
  __runFork () {
    ...
    this.master.on('exit', () => {
      // 解绑主进程退出了
      this.master.off('exit', this.__runForkEventExit)
      // 解绑主进程关闭了
      this.master.off('close', this.__runForkEventClose)
      // 解绑主进程出错了
      this.master.off('error', this.__runForkEventError)
      // 解绑主进程系统消息
      this.master.off('message', this.__runForkEventMessage)
    })
    // 主进程关闭了

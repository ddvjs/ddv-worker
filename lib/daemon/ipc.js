  ...
  __ddvEventInit  () {
      ...
      if (this._processCallback && this._processCallback[res.id]) {
        if (res.errorStack) {
          if (util.isFunction(this._processCallback[res.id][1])) {
            let e = new this.DaemonError(res.message || 'daemon call master fail')
            e.stack = (res.errorStack || '') + e.stack
            e.type = res.errorType || 'callMaster'
            e.name = res.errorName || 'DAEMON_CALL_MASTER_FAIL'
            e.id = res.id
            e.handle = handle
    ...
    this.on('master::event::callDaemon', (res, handle) => {
      ...
      Promise.resolve().then(() => {
        ...
        if (this._onMasterCalls[res.name] instanceof Promise) {
          PromiseRes.push(this._onMasterCalls[res.name](res.data, handle, callback))
        } else {
          this._onMasterCalls[res.name](res.data, handle, callback)
        }
        callback = void 0

...
worker.serverListenInit = function server () {
  ...
  process.nextTick(() => {
    worker.serverListenPort()
  })
}
worker.serverListenPort = function serverListenPort () {
  var argv, port, portIndex
  if (!worker.isHasMaster) {
    argv = process.argv
    portIndex = argv.indexOf('--port')
    port = 80
    port = portIndex > -1 ? parseInt(argv[portIndex + 1] || port) || port : port
    worker.server.listen({ port: port }, function () {
      console.log('Can not find the management process, automatically listening port ' + port)
    })
  }
}

...
worker.updateServerConf = function (serverConf) {
  ...
  if (worker.isHasMaster) {
    ...
    // 更新失败
    return Promise.reject(new Error(config['not_supported_update']))
  }
}
/** 管理进程的事件结束 **/
/**
 ...
 */
worker.onMasterCall('processInfo', function processInfoCb (data, handle) {
  return worker.getProcessInfo(data => ({data}))
})

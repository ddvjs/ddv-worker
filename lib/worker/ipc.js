...
const onMasterCalls = worker._onMasterCalls = Object.create(null)
...
worker.callMaster = function (name, data, handle, options, callback) {
  ...
  body.name = name
  body.data = data
  processCallback[id] = callback
  worker.sendToMaster('callMaster', body, handle, function (state, message) {
    if (state !== true && processCallback[id] &&
        util.isFunction(processCallback[id])) {
      processCallback[id](state, message)
      delete processCallback[id]
    }
    body = void 0
  })
...
worker.on('master::event::callWorker', function (res, handle) {
  ...
  Promise.resolve().then(() => {
    ...
    worker.sendToMaster('callWorkerRes', { id: res.id, data }, handle, options)
  }, e => {

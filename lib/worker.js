...
// 导出模块
module.exports = function workerExports (worker) {
  // 系统内部配置
  const sys = worker.__sys_private__ = Object.create(null)
  // 发信息个主进程的结果回调存储容器
  const processCallback = sys.processCallback = Object.create(null)
  // 长连接总数
  worker.socketSum = 0
  // webSocket 长连接总数
...
// 通过 socket 触发服务
  sys.serverSocketEmit = function serverSocketEmit (message, socket) {
    process.nextTick(function socketEmitNextTick () {
      if (!(worker.serverRun && util.isFunction(worker.serverRun))) {
...
// 运行
  ...
  sys.masterPingInit = function masterPingInit () {
    ...
    sys.masterPingLastTime = util.now()
    sys.setIntervalHandle = setInterval(sys.masterPing, worker.masterPingTimeOut)
  }
  sys.masterPing = function masterPing () {
    // 如果没有主进程
    if (worker.isHasMaster !== true) {
      return
    }
    if ((util.now() - (sys.masterPingLastTime || 0)) > (worker.masterPingTimeOut * 2)) {
